...
TrackerClusterGroup g_tracker_servers = {0, NULL, NULL, NULL};
...
static int tracker_mem_init_groups(FDFSGroups *pGroups)
{
    ...
    bytes = sizeof(FDFSGroupInfo *) * pGroups->alloc_size;
	pGroups->groups = (FDFSGroupInfo **)malloc(bytes);
	...
	pGroups->sorted_groups = (FDFSGroupInfo **)malloc(bytes);
	...
	memset(pGroups->sorted_groups, 0, bytes);
...
static int tracker_mem_realloc_groups(FDFSGroups *pGroups, const bool bNeedSleep)
{
    ...
    bytes = sizeof(FDFSGroupInfo *) * new_size;
	new_groups = (FDFSGroupInfo **)malloc(bytes);
	if (new_groups == NULL)
	{
		logError("file: "__FILE__", line: %d, "
			"malloc %d bytes fail", __LINE__, bytes);
	...
	new_sorted_groups = (FDFSGroupInfo **)malloc(bytes);
	...
	memset(new_sorted_groups, 0, bytes);
		...
		const bool bNeedSleep)
{
	...
	for (ppServer=new_servers+pGroup->storage_count;
            ppServer<ppServerEnd; ppServer++)
    {
        *ppServer = (FDFSStorageDetail *)malloc(
                sizeof(FDFSStorageDetail));
	...
	do
	{
		...
		*ppStorageServer = *(pGroup->all_servers + pGroup->storage_count);
        ...
        if (g_use_storage_id)
        {
            ...
            if ((*ppStorageServer)->rw_mode & R_OK)
            {
                pGroup->readable_count++;
            }
            if ((*ppStorageServer)->rw_mode & W_OK)
            {
                pGroup->writable_count++;
            }
        }
        else
        {
...
static TrackerClusterServer *tracker_group_get_server(
        const char *target_ip, const int target_port)
{
    ...
    {
        if (fdfs_server_contain(&(*ppServer)->server, target_ip, target_port))
        {
            return *ppServer;
        }
    }
...
static int find_my_ip_in_tracker_list()
{
    ...
    {
        pServer = tracker_group_get_server(current_ip, SF_G_INNER_PORT);
        if (pServer != NULL)
        {
            g_tracker_servers.myself = pServer;
            if (pServer->server.count > 1)
            {
                ConnectionInfo *conn;
                ConnectionInfo *end;

                end = pServer->server.connections + pServer->server.count;
                for (conn=pServer->server.connections; conn<end; conn++)
                {
        ...
        tracker_servers, const int tracker_count)
{
            ...
            ppLocalTracker<ppLocalEnd; pTrackerServer++, ppLocalTracker++)
    {
        pClusterServer = (TrackerClusterServer *)
            malloc(sizeof(TrackerClusterServer));
        if (pClusterServer == NULL)
        {
            logError("file: "__FILE__", line: %d, "
                    "malloc %d bytes fail, "
                    "errno: %d, error info: %s",
                    __LINE__, (int)sizeof(TrackerClusterServer),
                    errno, STRERROR(errno));
            return errno != 0 ? errno : ENOMEM;
        }
        ...
        tracker_servers, const int tracker_count)
{
	...
	TrackerClusterServer **new_servers;
    TrackerClusterServer *pClusterServer;
    ...
    for (pJoinTracker=tracker_servers; pJoinTracker<pJoinEnd; pJoinTracker++)
    {
        ...
        {
            if (fdfs_server_equal(pJoinTracker, &(*ppLocalTracker)->server))
            {
                break;
            }
            if (fdfs_server_contain_ex(pJoinTracker, &(*ppLocalTracker)->server))
            {
                fdfs_server_info_to_string(pJoinTracker,
                        ip_str_join, sizeof(ip_str_join));
                fdfs_server_info_to_string(&(*ppLocalTracker)->server,
                ...
                if (pJoinTracker->count > (*ppLocalTracker)->server.count)
                {
                    if (tracker_mem_copy_uniq_tracker_servers(pJoinTracker,
                                &(*ppLocalTracker)->server) == 0)
                    {
                        fdfs_server_info_to_string(&(*ppLocalTracker)->server,
	...
	bytes = sizeof(TrackerClusterServer *) *
        (g_tracker_servers.server_count + add_count);
	new_servers = (TrackerClusterServer **)malloc(bytes);
	if (new_servers == NULL)
	{
		logError("file: "__FILE__", line: %d, "
			"malloc %d bytes fail, "
	...
	memcpy(new_servers, g_tracker_servers.servers,
            sizeof(TrackerClusterServer *) *
	...
	for (pJoinTracker=tracker_servers; pJoinTracker<pJoinEnd; pJoinTracker++)
	{
        ...
        {
            if (fdfs_server_contain_ex(pJoinTracker,
                        &(*ppLocalTracker)->server))
            {
                break;
            }
        }

		if (ppLocalTracker == ppNewServer)
        {
            pClusterServer = (TrackerClusterServer *)
                malloc(sizeof(TrackerClusterServer));
            if (pClusterServer == NULL)
	...
	add_count = (ppNewServer - new_servers) - g_tracker_servers.server_count;
        ...
        const char *ip_addr, const int port, const char *storage_id)
{
    if (g_use_storage_id)
    {
        ...
        {
            pStorageIdInfo = fdfs_get_storage_by_id(storage_id);
            if (pStorageIdInfo == NULL)
            {
                logError("file: "__FILE__", line: %d, "
            ...
            if (strcmp(pGroup->group_name, pStorageIdInfo->group_name) != 0)
            {
                logError("file: "__FILE__", line: %d, "
                        "storage id: %s, group_name: %s != %s in "
                        "storage_ids.conf", __LINE__, storage_id,
                        pGroup->group_name, pStorageIdInfo->group_name);
                return EINVAL;
            }
        }
        else
    ...
    else
    {
        ...
        for (ppServer=pGroup->all_servers; ppServer<ppEnd; ppServer++)
        {
            if ((*ppServer)->status == FDFS_STORAGE_STATUS_DELETED ||
                    (*ppServer)->status == FDFS_STORAGE_STATUS_IP_CHANGED ||
                    (*ppServer)->status == FDFS_STORAGE_STATUS_NONE)
            {
                continue;
            }

            if (strcmp(FDFS_FIRST_IP_ADDR(*ppServer), ip_addr) == 0 &&
                    (*ppServer)->storage_port == port)
            {
                return 0;
            }
        }
...
static int storage_join_get_storage_id(const char *ip_addr,
        FDFSStorageJoinBody *pJoinBody, FDFSStorageId *storage_id)
{
	if (g_use_storage_id)
	{
        FDFSStorageIdInfo *pStorageIdInfo;
        if (g_trust_storage_server_id && *(pJoinBody->storage_id) != '\0')
        {
            ...
            if (strcmp(pJoinBody->group_name, pStorageIdInfo->group_name) != 0)
            {
                logError("file: "__FILE__", line: %d, "
                        "storage id: %s, group_name: %s != %s in "
                        "storage_ids.conf", __LINE__, pJoinBody->storage_id,
                        pJoinBody->group_name, pStorageIdInfo->group_name);
                return EINVAL;
            }
        }
        else
        {
            pStorageIdInfo = fdfs_get_storage_id_by_group_and_ip_port(
                    pJoinBody->group_name, ip_addr, pJoinBody->storage_port);
            if (pStorageIdInfo == NULL)
            {
                logError("file: "__FILE__", line: %d, "
                        "get storage id info fail, group_name: %s, "
                        "storage ip: %s", __LINE__,
                        pJoinBody->group_name, ip_addr);
                return ENOENT;
            }
        }
...
static int storage_join_check_add_tracker_servers(FDFSStorageJoinBody *pJoinBody)
{
	int result;
		...
		const bool bNeedSleep)
{
    ...
    if ((result=storage_join_get_storage_id(ip_addr,
                    pJoinBody, &storage_id)) != 0)
    {
        return result;
    }

	tracker_mem_file_lock();
    result = storage_join_check_add_tracker_servers(pJoinBody);
	tracker_mem_file_unlock();
    if (result != 0)
    {
        return result;
...
static int tracker_mem_find_trunk_server_ex(FDFSGroupInfo *pGroup,
		const bool save, const int wait_seconds)
{
    ...
    if (pGroup->writable_storages.count < pGroup->writable_count)
    {
        if (wait_seconds > 0)
        {
            PTHREAD_MUTEX_UNLOCK(&mem_thread_lock);
            ...
            if (pGroup->pStoreServer != pStoreServer)
            {
                return EBUSY;
            }
        }
...
static int tracker_mem_find_trunk_server(FDFSGroupInfo *pGroup,
		const bool save)
{
    const int wait_seconds = 0;
    return tracker_mem_find_trunk_server_ex(pGroup, save, wait_seconds);
}
			...
			FDFSStorageDetail *pTargetServer) 
{
    ...
    if (pTargetServer->rw_mode & W_OK)
    {
        ...
        {
            tracker_mem_find_trunk_server_ex(pGroup, true,
                    (SF_G_NETWORK_TIMEOUT + 1) * 2 / 3);
        }
    }
